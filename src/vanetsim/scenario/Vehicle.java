		...
		if(active_){
			...
			if(curWaitTime_ == 0 && curStreet_ != null){
				...
				if(!isWaitingForSignal_() && curSpeed_ == 0)
				{
					...
					if(accuBlockingTime_ >= PRIORITY_TIMEOUT)
					{
						int times = accuBlockingTime_ / PRIORITY_TIMEOUT;
						
						for(int i=0;i<times;i++)  setPriorityInc();
					}
				}
...
//				}
	...
	private final boolean checkPolitness(int lane){
		/*
		//Noch auszubessern: in checkLaneFree() die neededDistance wieder zur鰕ksetzen
...
*/
	...
	//	System.out.println("distance: " + distance);
		...
		if(b2 != null){
			...
			//System.out.println("distanz ben魩igt: " + b2.curSpeed_ * t);
	...
	/**
	 * The attacked vehicle can咨 be found. But it can咨 be far because we received a beacon the last time.
