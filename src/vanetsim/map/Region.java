...
public final class Region{
	...
	public static ArrayList<TrafficLight> trafficLights_ = new ArrayList<TrafficLight>();
	
	public ArrayList<TrafficLight> getRegionAllTrafficLights(){return trafficLights_;}
	...
	//	VanetSimStart.getTextArea().append("node節點長度"+String.valueOf(length)+"\n");
		
		for(int i = 0; i < nodes_.length; ++i){
			
			nodes_[i].calculateJunction();		
			
//			VanetSimStart.getTextArea().append("| 節點ID "+nodes_[i].getNodeID()+"\n");
//			Debug.debugInfo("TAG", "| 節點ID "+nodes_[i].getNodeID()+"\n", true);
//			VanetSimStart.getTextArea().append("| 節點座標  ( "+nodes_[i].getNodeX() +" , "+nodes_[i].getNodeY() + " ) \n");
//			Debug.debugInfo("TAG", "| 節點座標  ( "+nodes_[i].getNodeX() +" , "+nodes_[i].getNodeY() + " ) \n", true);
			...
			{
				trafficLight_ = new TrafficLight(nodes_[i].getJunction());
				nodes_[i].setTrafficLight_(trafficLight_);
				trafficLights.add(trafficLight_);
			}
			
//			VanetSimStart.getTextArea().append("| 路口 "+nodes_[i].getJunction()+"\n");
//			Debug.debugInfo("TAG", "| 路口 "+nodes_[i].getJunction()+"\n", true);
			if(nodes_[i].getJunction() != null)
			{
				Street[] streets = nodes_[i].getJunction().getPriorityStreets();
				for(Street s : streets)
				{
//					VanetSimStart.getTextArea().append("| 道路名稱 "+s.getName()+"\n");
//					Debug.debugInfo("TAG", "| 道路名稱 "+s.getName()+"\n", true);
				}
			}
//			VanetSimStart.getTextArea().append("| 紅綠燈 "+nodes_[i].getTrafficLight_()+"\n");
//			Debug.debugInfo("TAG", "| 紅綠燈 "+nodes_[i].getTrafficLight_()+"\n", true);
			if(nodes_[i].getTrafficLight_() != null)
			{
//				VanetSimStart.getTextArea().append("| 紅綠燈目前燈號 "+nodes_[i].getTrafficLight_().getStateString()+"\n");
//				Debug.debugInfo("TAG", "| 紅綠燈目前燈號 "+nodes_[i].getTrafficLight_().getStateString()+"\n", true);
			}
				...
				if(!blackListed) blackList.add(street);
					...
					//	System.out.println(resultForSqrt);
						...
						//one solution
							...
							//if(street.getStartNode().getX() == 38191) System.out.println("gr银er");
