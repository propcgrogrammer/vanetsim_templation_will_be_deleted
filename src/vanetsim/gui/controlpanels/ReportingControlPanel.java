...
public final class ReportingControlPanel extends JPanel implements ActionListener, ItemListener{
	...
	/** FileName to check for vehicle file */
	...
	private void writeVehicleFile(Vehicle vehicle, String max)
	{
		FileWriter fw;
		...
		try {
			...
			{
			     FILENAME = "vehicle_" + new java.util.Date().getTime() + ".txt";
			}
			    
			    
			    String id = vehicle.getHexID();
			    String isActive = String.valueOf(vehicle.isActive());
			    String isEmergencyVehicle = String.valueOf(vehicle.isEmergencyVehicle());
			    String isWaitingForSignal = String.valueOf(vehicle.isWaitingForSignal_());
			    String curSpeed = String.valueOf(vehicle.getCurSpeed());
			    String curWaitTime = String.valueOf(vehicle.getCurWaitTime());
			...
			fw.write("|    車輛識別號                   |   " + id + "                 |\r\n");
			fw.write("+--------------------------------------+-----------------------------------------+\r\n");
			fw.write("|    車輛啟動                   |   " + isActive + "                 |\r\n");
			fw.write("+--------------------------------------+-----------------------------------------+\r\n");
			fw.write("|    緊急車輛                   |   " + isEmergencyVehicle + "                 |\r\n");
			fw.write("+--------------------------------------+-----------------------------------------+\r\n");
			fw.write("|    等待紅綠燈                   |   " + isWaitingForSignal + "                 |\r\n");
			fw.write("+--------------------------------------+-----------------------------------------+\r\n");
			fw.write("|    目前速度                   |   " + curSpeed + "                 |\r\n");
			fw.write("+--------------------------------------+-----------------------------------------+\r\n");
			fw.write("|    目前等待時間                   |   " + curWaitTime + "                 |\r\n");
			fw.write("+--------------------------------------+-----------------------------------------+\r\n");
		...
		for(i = 0; i < regions.length; ++i){
			for(j = 0; j < regions[i].length; ++j){
				...
				for(k = 0; k < vehicles.length; ++k){
					...
					writeVehicleFile(vehicle, String.valueOf(vehicles.length));
